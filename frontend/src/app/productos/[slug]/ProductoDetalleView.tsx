...
export default function ProductoDetalleView({ producto, relacionados }: Props) {
    ...
    useEffect(() => {
        ...
        if (selectedVariant && selectedVariant.indice_imagen !== undefined && selectedVariant.indice_imagen !== null) {
            // Verificamos que esa imagen exista en el array
            if (producto.imagen && producto.imagen[selectedVariant.indice_imagen]) {
                setSelectedImage(producto.imagen[selectedVariant.indice_imagen].url);
            }
        }
        ...
        <div className="min-h-screen bg-[#F9F7F2] text-[#1a1a1a] pb-20">
            ...
            <div className="container mx-auto px-4 pt-10 lg:pt-16">
                <nav className="text-xs text-gray-500 mb-8 uppercase tracking-wide font-medium">
                    <Link href="/">Home</Link> / <Link href="/productos">Productos</Link> / <span className="text-gray-900">{producto.nombre}</span>
                </nav>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
                    ...
                    <div className="lg:col-span-7 flex flex-col gap-4">
                        <div className="relative w-full aspect-square bg-white rounded-xl overflow-hidden shadow-sm">
                            {selectedImage ? (
                                <Image src={getImageUrl(selectedImage)} alt={producto.nombre} fill className="object-cover" priority />
                            ) : (
                            ...
                            <div className="flex gap-4 overflow-x-auto pb-2 scrollbar-hide">
                                {producto.imagen.map((img) => (
                                    <button
                                        ...
                                        className={`relative w-24 h-24 flex-shrink-0 rounded-lg overflow-hidden border-2 transition-all ${selectedImage === img.url ? "border-[#4A4A40]" : "border-transparent hover:border-gray-300"
                                            }`}
                                    >
                    ...
                    <div className="lg:col-span-5 flex flex-col pt-2">
                        ...
                        <div className="mb-6">
                            ...
                            <Link href="/simulador" className="text-sm font-medium underline underline-offset-4 hover:text-[#4A4A40]">
                                Grabado de virola
                            </Link>
                        </div>
                        ...
                        {producto.variantes && producto.variantes.length > 0 && (
                            <div className="mb-8">
                                <p className="text-sm font-bold text-gray-900 mb-3 uppercase tracking-wider">Elige una opción:</p>
                                <div className="flex flex-wrap gap-3">
                                    {producto.variantes.map((variant) => (
                                        <button
                                            key={variant.id}
                                            onClick={() => setSelectedVariant(variant)}
                                            className={`px-4 py-2 rounded-full text-sm font-medium border transition-all flex items-center gap-2 ${selectedVariant?.id === variant.id
                                        ...
                                        >
                                            ...
                                            {variant.codigo_color && (
                                                <span
                                                    className={`w-3 h-3 rounded-full border border-gray-300 shadow-sm ${selectedVariant?.id === variant.id ? 'border-white' : ''}`}
                                ...
                                {selectedVariant && !isOutOfStock && (
                                    <p className="text-xs text-gray-500 mt-2">Stock disponible: {selectedVariant.stock} u.</p>
                                )}
                            </div>
                        )}
                        ...
                        <div className="flex items-center gap-4 mb-8">
                            <div className={`flex items-center border border-gray-300 rounded-full px-4 py-3 bg-white shadow-sm ${isOutOfStock ? 'opacity-50 pointer-events-none' : ''}`}>
                                <button onClick={handleRestar} className="text-gray-500 hover:text-black px-2 text-lg">-</button>
                                <span className="w-8 text-center text-sm font-semibold">{cantidad}</span>
                                <button onClick={handleSumar} className={`text-gray-500 px-2 text-lg ${cantidad >= currentStock ? 'opacity-30 cursor-not-allowed' : 'hover:text-black'}`}>+</button>
                            </div>
            ...
            <div className="container mx-auto px-4 mt-24 max-w-6xl border-t border-gray-200 pt-16">
                ...
                {producto.opinions && producto.opinions.length > 0 ? (
                    <>
                        <div className="flex items-center gap-3 mb-8">
                            <span className="text-5xl font-medium text-[#1a1a1a]">
                                {(producto.opinions.reduce((acc, op) => acc + (op.Puntuacion || 0), 0) / producto.opinions.length).toFixed(1)}
                            </span>
                            <div className="flex flex-col">
                                <div className="flex text-[#4A4A40] text-lg">★★★★☆</div>
                                <span className="text-xs text-gray-500">{producto.opinions.length} calificaciones</span>
                            </div>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-16 gap-y-10 mb-8 mt-8">
                            {producto.opinions.slice(0, showAllReviews ? producto.opinions.length : 2).map((review) => (
                                <div key={review.id} className="border-b border-gray-100 pb-4">
                                    <div className="flex text-[#4A4A40] text-sm mb-1">
                                        {"★".repeat(review.Puntuacion || 5)}{"☆".repeat(5 - (review.Puntuacion || 5))}
                                    </div>
                                    <p className="text-gray-900 font-medium mb-1 text-sm italic">"{review.Texto}"</p>
                                    <p className="text-[10px] text-gray-400 uppercase tracking-wider">
                                        {review.Autor} {review.createdAt && ` - ${new Date(review.createdAt).toLocaleDateString()}`}
                                    </p>
                                </div>
                            ))}
                        </div>
                    ...
                    <p className="text-gray-500 italic mt-4">Este producto aún no tiene opiniones. ¡Sé el primero en comentar!</p>
                )}
            </div>

            {/* RELACIONADOS (Usamos ProductCard) */}
            <div className="container mx-auto px-4 mt-24 max-w-6xl pb-12">
                <h3 className="text-center text-xl font-bold mb-10 text-[#1a1a1a]">También te podría interesar...</h3>
